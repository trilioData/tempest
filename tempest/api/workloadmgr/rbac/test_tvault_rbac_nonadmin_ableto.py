	...
	try:
	    ...
	    # Create workload
	        ...
	        LOG.debug("Workload created successfully")
                reporting.add_test_step("Verification of workload creation", tvaultconf.PASS)
                reporting.set_test_script_status(tvaultconf.PASS)
	    else:
		LOG.debug("workload creation unsuccessful")
                reporting.add_test_step("Verification of workload creation", tvaultconf.FAIL)
                raise Exception ("RBAC policy fails for workload creation by non-admin user")
	    ...
	    # Create full snapshot 
            ...
            snapshot_status  = self.wait_for_snapshot_tobe_available(self.wid, self.snapshot_id)    
            if snapshot_status == "available":
                LOG.debug("snapshot created successfully")
                reporting.add_test_step("Verification of snapshot creation", tvaultconf.PASS)
                reporting.set_test_script_status(tvaultconf.PASS)
            else:
                LOG.debug("snapshot creation unsuccessful")
                reporting.add_test_step("Verification of snapshot creation", tvaultconf.FAIL)
                raise Exception ("RBAC policy fails for snapshot creation by non-admin user")
	    ...
	    restore_status = ""
            ...
            restore_id = self.snapshot_restore(self.wid, self.snapshot_id, restore_name=restore_name)
            restore_status = query_data.get_snapshot_restore_status(restore_name, self.snapshot_id)
            LOG.debug("Snapshot restore status initial: " + str(restore_status))
		...
		LOG.debug("snapshot Mount unsuccessful")
                reporting.add_test_step("Verification of snapshot mount", tvaultconf.FAIL)
	    ...
	    for snapshot_id in filecount_in_snapshots.keys():
		    ...
		    LOG.debug("Filepath Search unsuccessful")
	   	    ...
	   	    raise Exception ("Filesearch path does not execute correctly by non-admin user")
