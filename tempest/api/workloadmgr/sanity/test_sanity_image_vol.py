...
class WorkloadTest(base.BaseWorkloadmgrTest):
    ...
    def test_1_create_workload(self):
        try:
            ...
            reporting.add_test_script(str(counter)+". "+str(__name__)+ "_create_workload_" + time.strftime("%Y%m%d-%H%M%S"))
    ...
    def test_4_inplace_restore(self):
        try:
            ...
            time.sleep(40)
            #fl_ip = self.assign_floating_ips(vm_list[0])
            #LOG.debug("Floating ip assigned to inplace restore vm -> "+str(fl_ip))
            md5sums_dir_after = {}
            ssh = self.SshRemoteMachineConnectionWithRSAKey(str(floating_ip))
