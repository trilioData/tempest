...
class WorkloadsTest(base.BaseWorkloadmgrTest):
    ...
    def test_1_filesearch_default_parameters(self):
        ...
        try:
            ...
            LOG.debug("global parameters: {0} {1} {2} {3} {4} {5} {6}".format(str(instances_ids), str(
                snapshot_ids), str(date_from), str(date_to), str(wid), str(volumes_ids), str(security_group_id)))
            filecount_in_snapshots = {
                snapshot_ids[0]: 0, snapshot_ids[1]: 1, snapshot_ids[2]: 1, snapshot_ids[3]: 1}
            ...
            for snapshot_id in filecount_in_snapshots.keys():
                if snapshot_wise_filecount[snapshot_id] == filecount_in_snapshots[snapshot_id]:
                    ...
                    reporting.add_test_step(
                        "Verification of Filepath serach default_parameters", tvaultconf.FAIL)
                    raise Exception(
            ...
            if filesearch_status == True:
                ...
                reporting.add_test_step(
                    "Verification of Filepath serach default_parameters", tvaultconf.PASS)
    ...
    def test_2_filesearch_snapshotids(self):
        ...
        try:
            ...
            LOG.debug("gloabal parameters: {0} {1} {2} {3} {4} {5} {6}".format(str(instances_ids), str(
                snapshot_ids), str(date_from), str(date_to), str(wid), str(volumes_ids), str(security_group_id)))
            ...
            filecount_in_snapshots = {
                snapshot_ids[0]: 0, snapshot_ids[1]: 0, snapshot_ids[2]: 0, snapshot_ids[3]: 1}
            filesearch_id = self.filepath_search(
            ...
            if filesearch_status == True:
                ...
                reporting.add_test_step(
                    "Verification of Filepath serach with snapshotids", tvaultconf.PASS)
                reporting.test_case_to_write()
    ...
    def test_3_filesearch_firsttwosnapshots(self):
        ...
        try:
            ...
            filecount_in_snapshots = {
                snapshot_ids[0]: 0, snapshot_ids[1]: 0, snapshot_ids[2]: 0, snapshot_ids[3]: 1}
            filesearch_id = self.filepath_search(
                vmid_to_search, filepath_to_search, snapshot_ids_tosearch, start_snapshot, end_snapshot)
            snapshot_wise_filecount = self.verifyFilepath_Search(
                filesearch_id, filepath_to_search)
            for snapshot_id in snapshot_wise_filecount.keys():
                if filecount_in_snapshots[snapshot_id] == snapshot_wise_filecount[snapshot_id] and snapshot_id in snapshot_ids[:2]:
                    ...
                    reporting.add_test_step(
                        "Verification of Filepath serach for firsttwosnapshots", tvaultconf.FAIL)
                    raise Exception(
            ...
            if filesearch_status == True:
                ...
                reporting.add_test_step(
                    "Verification of Filepath serach for firsttwosnapshots", tvaultconf.PASS)
                reporting.test_case_to_write()
        ...
        try:
            ...
            for snapshot_id in snapshot_wise_filecount.keys():
                if filecount_in_snapshots[snapshot_id] == snapshot_wise_filecount[snapshot_id] and snapshot_id in snapshot_ids[2:]:
                    ...
                    reporting.add_test_step(
                        "Verification of Filepath serach for latesttwosnapshots", tvaultconf.FAIL)
                    raise Exception(
            ...
            if filesearch_status == True:
                ...
                reporting.add_test_step(
                    "Verification of Filepath serach for latesttwosnapshots", tvaultconf.PASS)
                reporting.test_case_to_write()
    ...
    def test_5_filesearch_daterange(self):
        ...
        try:
            ...
            filecount_in_snapshots = {
                snapshot_ids[0]: 0, snapshot_ids[1]: 0, snapshot_ids[2]: 0, snapshot_ids[3]: 1}
            filesearch_id = self.filepath_search(
                vmid_to_search, filepath_to_search, snapshot_ids_tosearch, start_snapshot, end_snapshot, date_from, date_to)
            snapshot_wise_filecount = self.verifyFilepath_Search(
                filesearch_id, filepath_to_search)
            for snapshot_id in snapshot_wise_filecount.keys():
            ...
            if filesearch_status == True:
                ...
                reporting.add_test_step(
                    "Verification of Filepath serach with daterange", tvaultconf.PASS)
                reporting.test_case_to_write()
    ...
    def test_6_filesearch_wildcard_star(self):
        ...
        try:
            ...
            filecount_in_snapshots = {
                snapshot_ids[0]: 0, snapshot_ids[1]: 0, snapshot_ids[2]: 2, snapshot_ids[3]: 2}
            filesearch_id = self.filepath_search(
                vmid_to_search, filepath_to_search)
            snapshot_wise_filecount = self.verifyFilepath_Search(
                filesearch_id, filepath_to_search)
            for snapshot_id in filecount_in_snapshots.keys():
                if snapshot_wise_filecount[snapshot_id] == filecount_in_snapshots[snapshot_id]:
                    ...
                    reporting.add_test_step(
                        "Verification of Filepath serach with wildcard_star", tvaultconf.FAIL)
                    raise Exception(
            ...
            if filesearch_status == True:
                ...
                reporting.add_test_step(
                    "Verification of Filepath serach with wildcard_star", tvaultconf.PASS)
                reporting.test_case_to_write()
    ...
    def test_7_filesearch_wildcards_questionmark(self):
        ...
        try:
            ...
            filecount_in_snapshots = {
                snapshot_ids[0]: 0, snapshot_ids[1]: 2, snapshot_ids[2]: 2, snapshot_ids[3]: 2}
            filesearch_id = self.filepath_search(
                vmid_to_search, filepath_to_search)
            snapshot_wise_filecount = self.verifyFilepath_Search(
                filesearch_id, filepath_to_search)
            for snapshot_id in filecount_in_snapshots.keys():
                if snapshot_wise_filecount[snapshot_id] == filecount_in_snapshots[snapshot_id]:
                    ...
                    reporting.add_test_step(
                        "Verification of Filepath serach with wildcards_questionmark", tvaultconf.FAIL)
                    raise Exception(
            ...
            if filesearch_status == True:
                ...
                reporting.add_test_step(
                    "Verification of Filepath serach with wildcards_questionmark", tvaultconf.PASS)
