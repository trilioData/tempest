...
class WorkloadTest(base.BaseWorkloadmgrTest):
    ...
    def multiple_vms(self, vm_count, key_pairs, sec_groups):
        ...
        for each in range(1, vm_count+1):
            if each<=(vm_count/2):
                ...
                #Create instance
                kptouple=random.choice(key_pairs)
                vm_id = self.create_vm(security_group_id=random.choice(sec_groups), key_pair=kptouple[1], key_name=kptouple[0], block_mapping_data=block_mapping_details, vm_cleanup=True)
    ...
    def test_functional(self):
        try:
            ...
            for each in vms:
                if len(vms[each]) == 3:
                    ssh = self.SshRemoteMachineConnectionWithRSAKeyName(str(vms[each][2]), vms[each][0])
                    i = 0
            ...
            for workload_id in wls:
                i+=1
                incr_snapshot_id=self.workload_snapshot(workload_id, False, snapshot_cleanup=True)
                time.sleep(5)
                self.wait_for_workload_tobe_available(workload_id)
            ...
            for workload in workloads:
                ...
                if(self.getRestoreStatus(wid, snapshotid, restore_id_1) == "available"):
                    reporting.add_test_step("Selective restore-{}".format(i), tvaultconf.PASS)
                    LOG.debug('selective restore passed')
                else:
                    reporting.add_test_step("Selective restore-{}".format(i), tvaultconf.FAIL)
                ...
                for rvm in restored_vms:
                    ...
                    fip = ""
                    j = 0
                    rvmname = self.get_vm_details(rvm)['server']['name'].replace('_selectively_restored','')
