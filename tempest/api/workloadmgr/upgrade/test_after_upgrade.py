    ...
    def _compare_license(self):
        ...
        for key in upgrade_data_conf.license_details.keys():
            if self.license_after_upgrade:
                ...
                self.license_check_flag=False
                reporting.add_test_step("License not imported correctly", tvaultconf.FAIL)
                reporting.set_test_script_status(tvaultconf.FAIL)
        ...
        if self.settings_after_upgrade:
            for key in self.settings_before_upgrade.keys():
                if(str(self.settings_after_upgrade[key]) == str(self.settings_before_upgrade[key])):
                    ...
                    reporting.add_test_step("Email setting '" + str(key) + "' not preserve", tvaultconf.FAIL)
                    reporting.set_test_script_status(tvaultconf.FAIL)
        else:
            self.email_check_flag=False
    ...
    def test_after_upgrade(self):
        try:
            ...
            if self._compare_email_settings():
                reporting.add_test_step("Email settings imported correctly", tvaultconf.PASS)
            ...
            for i in range(0, len(self.data_before_upgrade)):
                self.workloads_before_upgrade.append(self.data_before_upgrade[i]['workload_id'])
            LOG.debug("Workloads before upgrade: " + str(self.workloads_before_upgrade))
            if(all(id in self.workloads for id in self.workloads_before_upgrade)):
                reporting.add_test_step("Verify imported workloads", tvaultconf.PASS)
            else:
                reporting.add_test_step("Verify imported workloads", tvaultconf.FAIL)
            ...
            for i in range(0, len(self.workloads_before_upgrade)):
                self.workload_instances_data = self.get_workload_details(self.workloads_before_upgrade[i])['instances']
                ...
                if(self.workload_members_after_upgrade.sort() == upgrade_data_conf.instance_id.sort()):
                    reporting.add_test_step("Verify workload members after import for workload " + str(i+1), tvaultconf.PASS)
                else:
                    reporting.add_test_step("Verify workload members after import for workload " + str(i+1), tvaultconf.FAIL)
                    reporting.set_test_script_status(tvaultconf.FAIL)

            #Verify if workload scheduler settings are preserved
            for i in range(0, len(self.workloads_before_upgrade)):
                self.scheduler_settings_check_flag = True
                self.scheduler_settings_before_upgrade = self.data_before_upgrade[i]['scheduler_settings']
                self.scheduler_settings_after_upgrade = self.getSchedulerDetails(self.data_before_upgrade[i]['workload_id'])
                ...
                for key in self.scheduler_settings_before_upgrade.keys():
                    if(key == 'nextrun'):
                        ...
                        if(self.scheduler_settings_before_upgrade[key] == self.scheduler_settings_after_upgrade[key]):
                            LOG.debug("Workload scheduler '" + str(key) + "' preserved for workload " + str(i+1))
                        else:
                            self.scheduler_settings_check_flag = False
                            reporting.add_test_step("Workload scheduler '" + str(key) + "' not preserved for workload " + str(i+1), tvaultconf.FAIL)
                            reporting.set_test_script_status(tvaultconf.FAIL)
                
                if self.scheduler_settings_check_flag:
                    reporting.add_test_step("Scheduler settings preserved for workload " + str(i+1), tvaultconf.PASS)
            ...
            for i in range(0, len(self.data_before_upgrade)):
                self.snapshots_before_upgrade.append(self.data_before_upgrade[i]['snapshot_id'])
            LOG.debug("Snapshots before upgrade: " + str(self.snapshots_before_upgrade))
            ...
            self.selective_restore_id = self.snapshot_selective_restore(self.data_before_upgrade[0]['workload_id'], self.data_before_upgrade[0]['snapshot_id'], instance_details=self.instance_details, network_details=self.network_details, restore_cleanup=False)
            LOG.debug("Selective Restore ID: " + str(self.selective_restore_id))
            if(self.selective_restore_id == 0):
                reporting.add_test_step("Trigger selective restore of imported snapshot for workload-1", tvaultconf.FAIL)
                reporting.set_test_script_status(tvaultconf.FAIL)
            else:
                self.wait_for_workload_tobe_available(self.data_before_upgrade[0]['workload_id'])
                if(self.getRestoreStatus(self.data_before_upgrade[0]['workload_id'], self.data_before_upgrade[0]['snapshot_id'], self.selective_restore_id) == "available"):
                    reporting.add_test_step("Selective restore of imported snapshot for workload-1", tvaultconf.PASS)
                else:
                    reporting.add_test_step("Selective restore of imported snapshot for workload-1", tvaultconf.FAIL)
                    reporting.set_test_script_status(tvaultconf.FAIL)
            ...
            self.workload_instances_after_selective_restore = self.get_workload_details(self.data_before_upgrade[0]['workload_id'])['instances']
            ...
            if(self.workload_members_after_selective_restore.sort() == upgrade_data_conf.instance_id.sort()):
                reporting.add_test_step("Verify workload members after selective restore for workload-1", tvaultconf.PASS)
            else:
                reporting.add_test_step("Verify workload members after selective restore for workload-1", tvaultconf.FAIL)
                reporting.set_test_script_status(tvaultconf.FAIL)
            ...
            self.oneclick_restore_id = self.snapshot_restore(self.data_before_upgrade[0]['workload_id'], self.data_before_upgrade[0]['snapshot_id'], restore_cleanup=False)
            ...
            if(self.getRestoreStatus(self.data_before_upgrade[0]['workload_id'], self.data_before_upgrade[0]['snapshot_id'], self.oneclick_restore_id) == "available"):
                reporting.add_test_step("One click restore of imported snapshot for workload-1", tvaultconf.PASS)
            else:
                reporting.add_test_step("One click restore of imported snapshot for workload-1", tvaultconf.FAIL)
            ...
            self.workload_instances_after_oneclick_restore = self.get_workload_details(self.data_before_upgrade[0]['workload_id'])['instances']
            ...
            if(self.workload_members_after_oneclick_restore.sort() == self.oneclick_restored_vms.sort()):
                reporting.add_test_step("Verify workload members after oneclick restore for workload-1", tvaultconf.PASS)
            else:
                reporting.add_test_step("Verify workload members after oneclick restore for workload-1", tvaultconf.FAIL)

            #Trigger full snapshot of imported workloads
            self.new_snapshots = []
            for i in range(0, len(self.workloads_before_upgrade)):
                self.new_snapshot_id = self.workload_snapshot(self.data_before_upgrade[i]['workload_id'], is_full=True, snapshot_cleanup=False)
                LOG.debug("New full snapshot id of imported workload " + str(i+1) + ": " + str(self.new_snapshot_id))
                self.new_snapshots.append(self.new_snapshot_id)
            
            for i in range(0, len(self.new_snapshots)):
                self.wait_for_workload_tobe_available(self.data_before_upgrade[i]['workload_id'])
                if(self.getSnapshotStatus(self.data_before_upgrade[i]['workload_id'], self.new_snapshots[i]) == "available"):
                    reporting.add_test_step("Create new snapshot of imported workload " + str(i+1), tvaultconf.PASS)
                else:
                    reporting.add_test_step("Create new snapshot of imported workload " + str(i+1), tvaultconf.FAIL)
                    reporting.set_test_script_status(tvaultconf.FAIL)

            #Cleanup
            #Delete selective restore
            self.restore_delete(self.data_before_upgrade[i]['workload_id'], self.data_before_upgrade[i]['snapshot_id'], self.selective_restore_id)
            self.delete_restored_vms(self.selective_restored_vms, self.selective_restored_volumes)

            #Delete one click restore
            self.restore_delete(self.data_before_upgrade[i]['workload_id'], self.data_before_upgrade[i]['snapshot_id'], self.oneclick_restore_id)
            self.delete_restored_vms(self.oneclick_restored_vms, self.oneclick_restored_volumes)

            #Delete new snapshots created
            for i in range(0, len(self.new_snapshots)):
                self.snapshot_delete(self.data_before_upgrade[i]['workload_id'], self.new_snapshots[i])

            #Delete imported snapshots and workloads
            for i in range(0, len(self.workloads_before_upgrade)):
                self.snapshot_delete(self.data_before_upgrade[i]['workload_id'], self.data_before_upgrade[i]['snapshot_id'])
                self.workload_delete(self.data_before_upgrade[i]['workload_id'])
