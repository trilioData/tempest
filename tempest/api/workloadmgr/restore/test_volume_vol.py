...
class WorkloadTest(base.BaseWorkloadmgrTest):
    ...
    def install_qemu_ga(self, flo_ip):
        ...
        commands = ["sudo sed -i '1i nameserver 8.8.8.8' /etc/resolv.conf",
    ...
    def test_1_volume_volume(self):
        try:
            ### VM and Workload ###
            tests = [['tempest.api.workloadmgr.restore.test_volume_vol_Selective-restore', 0],
                     ['tempest.api.workloadmgr.restore.test_volume_vol_Inplace-restore', 0], ['tempest.api.workloadmgr.restore.test_volume_vol_Oneclick-restore', 0]]
            ...
            vm_id = self.create_vm(
                key_pair=kp, image_id="", block_mapping_data=self.block_mapping_details, vm_cleanup=False)
            LOG.debug("VM ID : " + str(vm_id))
            time.sleep(30)

            # Create and attach volume
            volume_id = self.create_volume(
                volume_type_id=CONF.volume.volume_type_id, volume_cleanup=False)
            ...
            LOG.debug(
                "Volumes deleted successfully for one click restore : " + str(volume_id))
