...
class WorkloadTest(base.BaseWorkloadmgrTest):
    ...
    def install_qemu_ga(self, flo_ip):
        ...
        commands = ["sudo sed -i '1i nameserver 8.8.8.8' /etc/resolv.conf", "sudo apt-get update", "sudo apt-get install qemu-guest-agent"]
        for x in range(len(commands)):
            if x!=1:
                ssh = self.SshRemoteMachineConnectionWithRSAKey(str(flo_ip))
                LOG.debug("\nExecuting: {}\n".format(str(commands[x])))
                stdin, stdout, stderr = ssh.exec_command(commands[x], timeout=180)
                time.sleep(25)
                ssh.close()
            else:
                ssh = self.SshRemoteMachineConnectionWithRSAKey(str(flo_ip))
    ...
    def create_snapshot(self, workload_id, is_full=True):
        ...
        snapshot_id, command_execution, snapshot_execution = self.workload_snapshot_cli(workload_id, is_full=is_full)
        if command_execution == 'pass':
            reporting.add_test_step("{} snapshot command execution".format(substitution), tvaultconf.PASS)
            LOG.debug("Command executed correctly for full snapshot")
        else:
            reporting.add_test_step("{} snapshot command execution".format(substitution), tvaultconf.FAIL)
            raise Exception("Command did not execute correctly for full snapshot")

        if snapshot_execution == 'pass':
            reporting.add_test_step("{} snapshot".format(substitution), tvaultconf.PASS)
        else:
            reporting.add_test_step("{} snapshot".format(substitution), tvaultconf.FAIL)
    ...
    def test_1_volume_volume(self):
        try:
            ...
            md5sums_before_full = self.calcmd5sum(floating_ip_1, mount_points[0])
            LOG.debug("\nMD5SUM of the data before full snapshot : {}\n".format(md5sums_before_full))
            ...
            md5sums_before_incremental = self.calcmd5sum(floating_ip_1, mount_points[0])
            ...
            time.sleep(5)
            md5sums_after_selective = self.calcmd5sum(floating_ip_2, mount_points[0])
            ssh.close()
            ...
            time.sleep(5)
            md5sums_after_inplace = self.calcmd5sum(floating_ip_1, mount_points[0])
            ssh.close()
            ...
            self.execute_command_disk_mount(ssh, str(floating_ip_1), [volumes[0]], [mount_points[0]])
            time.sleep(5)
            md5sums_after_1clickrestore = self.calcmd5sum(floating_ip_1, mount_points[0])
